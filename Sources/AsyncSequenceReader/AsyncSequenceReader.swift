...
public class AsyncSequenceReader<
...
>: AsyncReadSequence {
    ...
    @usableFromInline
    let read: (_ iterator: inout AsyncBufferedIterator<BaseIterator, BaseFailure>) async throws(Failure) -> Element?
    
    @inlinable
    public init(
        _ baseIterator: AsyncBufferedIterator<BaseIterator, BaseFailure>,
        read: @escaping (_ iterator: inout AsyncBufferedIterator<BaseIterator, BaseFailure>) async throws(Failure) -> Element?
    ) {
