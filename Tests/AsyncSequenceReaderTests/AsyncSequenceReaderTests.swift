...
@Suite struct AsyncSequenceReaderTests {
    ...
    func countCharacters<ReadSequence: AsyncReadSequence>(_ sequence: sending ReadSequence) async throws -> sending Int? where ReadSequence.Element == String {
    ...
    @Test func transformTrapsForUnreadTestSequence() async throws {
        ...
        #if DEBUG
        #expect(result?.standardErrorUTF8Lines.first == "AsyncSequenceReader/AsyncReadSequence.swift:44: Precondition failed: A transform was requested, but the sequence was left in a state where the next value will never be read (fix: Use `sequence.makeBufferedIterator()` instead of calling `.transform` on `iterator` directly)")
        #endif
    }
